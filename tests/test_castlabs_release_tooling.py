...
def test_prerelease_version_is_consistent():
    assert release.project_version(ROOT) == "0.37.12.dev3"
    first_line = (
        (ROOT / "src" / "c2pa" / "c2pa.py").read_text(encoding="utf-8").splitlines()[13]
    )
    assert first_line == "# Version: 0.37.12.dev3"
...
def test_release_workflows_are_pinned_bounded_and_do_not_drift_from_helper(
    ...
    assert (
        "c2pa_python-0.37.12.dev3-py3-none-manylinux_2_28_x86_64.whl" in release_workflow
    )
    ...
    assert "if: github.ref == 'refs/tags/castlabs-v0.37.12.dev3'" in release_workflow
    ...
    assert "--source-ref refs/tags/castlabs-v0.37.12.dev3" in pypi_workflow
    assert '--source-digest "$SOURCE_SHA"' in pypi_workflow
    assert "pypi.org/pypi/c2pa-python/0.37.12.dev3/json" in pypi_workflow
    ...
    assert 'test "$VERSION" != 0.37.12.dev3' not in legacy_workflow
    ...
    assert release_workflow.count("CASTLABS_RELEASE_EXPECTED_VERSION: 0.37.12.dev3") == 2
    ...
    assert "## Version 0.37.12.dev3" in release_notes
...
def test_wheel_inspection_rejects_unsafe_and_multiple_native_members():
    with tempfile.TemporaryDirectory() as temp:
        root = Path(temp)
        wheel = root / "c2pa_python-0.37.12.dev3-py3-none-win_amd64.whl"
        ...
        unsafe = root / "c2pa_python-0.37.12.dev3-py3-none-manylinux_2_28_x86_64.whl"
